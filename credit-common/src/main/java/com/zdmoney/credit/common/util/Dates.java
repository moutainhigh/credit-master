...
public class Dates extends DateUtils {
    ...
    public static long diffDays(Date one, Date two) {
        ...
        try {
            one = dateFormat.parse(dateFormat.format(one));
            two = dateFormat.parse(dateFormat.format(two));
        } catch (ParseException e) {
            e.printStackTrace();
        }
    ...
    * 得到date 的上个月的最后一天 
    * @param date 参数日期
    * @param pattern 格式 例如 yyyyMMdd
    * @return
    */
    public static String getBeforeMonthLastDay(Date date,String pattern){
    	Calendar calendar=Calendar.getInstance();
    	calendar.setTime(date);
		int month=calendar.get(Calendar.MONTH);
		calendar.set(Calendar.MONTH, month-1);
		calendar.set(Calendar.DAY_OF_MONTH, calendar.getActualMaximum(Calendar.DAY_OF_MONTH));
		String str = Dates.getDateTime(calendar.getTime(),pattern);
		return str;
    }
}
