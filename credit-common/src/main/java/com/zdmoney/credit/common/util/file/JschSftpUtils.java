...
public class JschSftpUtils {
    ...
    /** 日志记录器  */
    private Logger logger = Logger.getLogger(JschSftpUtils.class);
    /** Session */
     ...
     * @param password 密码
     */
    public JschSftpUtils(String host,int port,int timeout,String username,String password){
        this.host = host;
        this.port = port;
     ...
     * @throws Exception
     */
    public InputStream downloadToIs(String directory, String downloadFile){
    	String currentDir = currentDir();   
        try {
            ...
            logger.info("file:{} is download successful");
            return is;
        } catch (Exception e) {
            logger.error("download file failed",e);
            return null;
