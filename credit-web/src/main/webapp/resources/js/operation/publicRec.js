$(function() {
    $.publicRec  = {
        /** 被认领客户信息表格数据源地址 **/
        dataGridUrl : global.contextPath + '/operation/searchPublicAccountReceiveInfo',
        /** 确认领取url **/
        confirmReceiveUrl : global.contextPath + '/operation/confirmReceive',
        /** 对公还款记录(前台选中的行对象) **/
        recRow : undefined,
        /** 被认领人信息窗口 **/
        ...
        reloadDataGrid : function() {
            if(!$.publicRec.validate()){
                return;
            }
            /** 获取查询表单数据转换成JSON对象 **/
            var searchMsg = $.publicRec.recSearchForm.serialize();
            /** 对参数进行解码(显示中文) **/
            searchMsg = decodeURIComponent(searchMsg);
            /** 字符串转换为对象 **/
            var queryParam = $.serializeToJsonObject(searchMsg);
            /** 追加url参数**/
            queryParam.url = $.publicRec.dataGridUrl;
            /** 查询并加载数据**/
            $.publicRec.recDataGrid.datagrid('reloadData',queryParam);
        },
        /** 查询校验 **/
        validate : function(){
            if(!$.publicRec.recSearchForm.form("validate")){
                return false;
            }
            if(!searchCheck()){
                return false;
    ...
    $.publicRec.pg = {
        'page' : 1,
        'rows' : $.publicRec.pageSize
    }
    
    /** DataGrid初始化 **/
    $.publicRec.recDataGrid.datagrid({
        pg : $.publicRec.pg,
        ...
        columns : [[{/** 列定义 **/
            ...
            formatter:function(value,row,index){
                if(value){
                    ...
                    var elements ="<a href='#' class='confirmReceive' loanId='"+row.loanId+"' borrowerName='"+ row.borrowerName +"' ></a>";
                    return elements;
                }
            }
        }]],
        /** 每页显示的记录条数，默认为10 **/
        pageSize : $.publicRec.pageSize,
        /** 可以设置每页记录条数的列表 **/
        pageList : $.publicRec.pageSizeList,
        /** 工具栏定义 **/
        toolbar : '#recTB',
        /** 自定义行样式 **/
        ...
        onLoadSuccess:function(data){
                ...
                onClick:function(){
                    ...
                    $.messager.confirm("提示","确认领取吗？",function(r){
                        if(r){
                            ...
                            $.ajaxPackage({
                                url:$.publicRec.confirmReceiveUrl,
                                type:"post",
                                data: json,
                                dataType:"json",
                                success:function(response, textStatus, jqXHR){
                                    ...
                                    setTimeout(function(){
                                        /** 关闭对公还款认领窗口 **/
                                        $.publicRec.showRectWin.window('close');
                                        /** 刷新对公还款页面数据记录 **/
                                        $.publicAccount.reloadDataGrid();
                                    }, 600);
                                },
            ...
            /** 页面自适应 **/
            $.publicRec.recDataGrid.datagrid('resize');
        }
    });
    
    /** 分页对象初始化 **/
    $.publicRec.pager = $.publicRec.recDataGrid.datagrid('getPager');
    $.publicRec.pager.pagination({
        onSelectPage : function(pageNumber,pageSize) {
            $.publicRec.pg.page = pageNumber;
            $.publicRec.pg.rows = pageSize;
            $.publicRec.reloadDataGrid();
        }
    });
    
    /** 被认领人信息窗口面板参数定义 **/
    $.publicRec.showRectWin.window({
        /** 窗口宽度 **/
    ...
    $('#recClearBtn').click(function() {
        $.publicRec.recSearchForm.form('reset');
    })
})
/** 查看客户详细资料 **/
