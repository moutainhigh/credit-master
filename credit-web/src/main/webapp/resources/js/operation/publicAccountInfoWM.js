$(function() {
    $.privateAccount = {
        /** 表格数据源地址 **/
        dataGridUrl : global.contextPath + '/operation/searchPublicAccountWMInfo',
        /** 对私导入url **/
        importUrl: global.contextPath + '/operation/importPublicAccountWMInfo',
        /** 对公导入（外贸2）url **/
        importUrl2: global.contextPath + '/operation/importPublicAccountWM2Info',
        /** 外贸3对私导入url **/
        importUrlWM3: global.contextPath + '/operation/importPrivateAccountWM3Info',
        /** 对公导入（华瑞渤海）url **/
        importUrlHRBH: global.contextPath + '/operation/importPublicAccountHRBHInfo',
        /** 对公导入（陆金所）url **/
        importLufaxFileUrl: global.contextPath + '/operation/importLufaxAccountFile',
        /** 对私导出已认领结果url **/
        exportReceiveInfoUrl: global.contextPath + '/operation/exportPrivateAccountReceiveInfo',
        /** 对私导出查询结果url **/
        exportSearchResultUrl: global.contextPath + '/operation/exportPrivateAccountSearchResult',
        /** 领取校验url **/
        receiveCheckUrl:global.contextPath + '/operation/publicAccountWMReceiveCheck',
//        /** 领取url **/
//        receiveInitUrl : global.contextPath + '/repayment/publicAccountReceiveInfoInit',
        /** 撤销url**/
        cancelUrl : global.contextPath + '/operation/cancelReceiveWm',
        /** 渠道确认url **/
        channelConfirmUrl : global.contextPath + '/operation/channelConfirmWm',
        /** 加载对公账户领取时间url **/
        loadReceiveTimeUrl : global.contextPath + '/operation/loadReceiveTimeWm',
        /** 修改对公账户领取时间url **/
        updateReceiveTimeUrl : global.contextPath + '/operation/updateReceiveTimeWm',
        /** 查询操作日志url **/
        searchLogInfoUrl : global.contextPath + '/operation/searchLogInfoWm',
        /** 对公账户信息表格对象 **/
        ...
        /** 对公导入（外贸2）**/
        baseFileForm2 : $("#baseFileForm2"),
        /** 对公导入（外贸3）**/
        baseFileFormWM3 : $("#baseFileFormWM3"),
        /** 对公导入（华瑞渤海）**/
        baseFileForm3 : $("#baseFileForm3"),
        /** 对公导入表单对象（陆金所）**/
        lufaxFileForm : $("#lufaxFileForm"),
        /** 修改领取时间面板对象 **/
        ...
        onLoadSuccess:function(data){
            ...
            $(".log").linkbutton({
                ...
                onClick:function(){
                    ...
                    url = url +"?objectId="+id;
                    url = url +"&logName=PublicAccountInfoWMController";
...
//        }
    ...
    $('#importBtn').click(function(){
        ...
        $.privateAccount.baseFileForm2.form('clear');
        $.privateAccount.baseFileFormWM3.form('clear');
        $.privateAccount.lufaxFileForm.form('clear');
    })
    ...
    /** 外贸3对私还款信息批量导入 **/
    $("#privateAccountImportWM3").click(function(){
        var file = $("#privateAccountfileWM3").filebox("getValue");
        if($.isEmpty(file)){
            $.messager.alert('警告','请选择导入文件！','warning');
            return;
        }
        $.messager.confirm("提示","确认批量导入吗？",function(r){
            if(r){
                $.privateAccount.baseFileFormWM3.ajaxSubmit({
                    type: "post",
                    dataType : 'json',
                    url: $.privateAccount.importUrlWM3,
                    hasDownloadFile : true,
                    success: function () {
                        $.messager.alert('信息','操作完成！','info');
                        setTimeout(function(){
    ...
    /** 对公还款（外贸2）信息批量导入 **/
    $("#privateAccountImport2").click(function(){
        var file = $("#privateAccountfile2").filebox("getValue");
        if($.isEmpty(file)){
            $.messager.alert('警告','请选择导入文件！','warning');
            return;
        }
        $.messager.confirm("提示","确认批量导入吗？",function(r){
            if(r){
                $.privateAccount.baseFileForm2.ajaxSubmit({
                    type: "post",
                    dataType : 'json',
                    url: $.privateAccount.importUrl2,
                    hasDownloadFile : true,
                    success: function () {
                        $.messager.alert('信息','操作完成！','info');
                        setTimeout(function(){
    ...
    /** 对公还款（华瑞渤海）信息批量导入 **/
    $("#privateAccountImport3").click(function(){
        var file = $("#privateAccountfile3").filebox("getValue");
        if($.isEmpty(file)){
            $.messager.alert('警告','请选择导入文件！','warning');
            return;
        }
        $.messager.confirm("提示","确认批量导入吗？",function(r){
            if(r){
                $.privateAccount.baseFileForm3.ajaxSubmit({
                    type: "post",
                    dataType : 'json',
                    url: $.privateAccount.importUrlHRBH,
                    hasDownloadFile : true,
                    success: function () {
                        $.messager.alert('信息','操作完成！','info');
                        setTimeout(function(){
    ...
    /** 对公还款（陆金所）银行流水信息批量导入 **/
    $("#importLufaxFileBtn").click(function(){
        // 导入文件
        var file = $("#lufaxAccountFile").filebox("getValue");
        if($.isEmpty(file)){
            $.messager.alert('警告','请选择导入文件！','warning');
            return;
        }
        $.messager.confirm("提示","确认批量导入吗？",function(r){
            if(r){
                $.privateAccount.lufaxFileForm.ajaxSubmit({
                    type: "post",
                    dataType : 'json',
                    url: $.privateAccount.importLufaxFileUrl,
                    hasDownloadFile : true,
                    success: function () {
                        $.messager.alert('信息','操作完成！','info');
                        setTimeout(function(){
