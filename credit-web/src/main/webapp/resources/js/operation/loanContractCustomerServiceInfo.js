$(function() {
    // 表格数据源地址
    var dataGridUrl = global.contextPath + '/operation/searchLoanContractCustomerServiceInfo';
    
    // 更新客服url
    var updateUrl = global.contextPath + '/operation/updateLoanContractCustomerServiceInfo';
    
    // 批量更新客服url
    var batchUpdateUrl = global.contextPath + '/operation/batchUpdateLoanContractCustomerServiceInfo';
    
    // 导出excel文件url
    var exportUrl = global.contextPath + '/operation/exportLoanContractCustomerServiceInfo';
    ...
    function check(){
        ...
        if($.isEmpty(name) && $.isEmpty(mobile) && $.isEmpty(idNum) && $.isEmpty(crmId)&& $.isEmpty(loanState)&& $.isEmpty(contractNum)){
            return false;
        }
        // 防止输入空白查询
        $("#name").val(name);
