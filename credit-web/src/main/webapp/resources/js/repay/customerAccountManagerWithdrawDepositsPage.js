	    ...
	    $("#submitButton").bind("click",function(envent){
	    	if($(this).linkbutton("options").disabled==false){
		    	if($("#conditionForm").form("validate")){
		    		...
		    		params.zhuxuedaiOrg = $("#params").data("zhuxuedaiOrg");
					$("#bottonBox .easyui-linkbutton").linkbutton("disable");
					$.ajaxPackage({
						url:global.contextPath+"/repay/customerAccountManagerWithdrawDeposits",
						async:true,
						type:"POST",
						data:params,
						dataType:"json",
						...
						error:function(response, textStatus, jqXHR){
							$.messager.alert("提示", "提现操作失败", "error");
						},
			...
			$.ajaxPackage({
				...
				success:function(response, textStatus, jqXHR){
					if(response.resCode == "000000"){
						...
						$("#canWithdrawDepositAmount").html( response.attachment.customerAccountInfo.withdrawDepositAmount+"");
					}else{
						$.messager.alert("提示", response.resMsg, "error");
					}
				},
