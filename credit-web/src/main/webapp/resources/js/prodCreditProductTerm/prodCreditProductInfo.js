$(function() {
	...
	var dataGridUrl = global.contextPath
			+ '/system/ProdCreditProductInfo/search';
	...
	var insertProdCreditProductInfo = global.contextPath
			+ '/system/ProdCreditProductInfo/saveOrUpdateProdCreditProductInfo';
	var loadDataProdCreditProductTerm = global.contextPath
			+ '/system/ProdCreditProductInfo/loadDataProdCreditProductInfo';

	var deleteProdCreditProductTerm = global.contextPath
			+ '/system/ProdCreditProductInfo/deleteProdCreditProductTermById';
	// 每页显示的记录条数，默认为10
	var pageSize = 10;
	// 设置每页记录条数的列表
	...
	function updateButEvent() {
		...
		if (selectedRow) {
			var id = selectedRow.term.productId;
			var termId = selectedRow.term.id;
			id=id+","+termId;
			$.ajaxPackage({
				type : 'get',
				...
				success : function(data, textStatus, jqXHR) {
					...
					if (resCode == '000000') {
						...
						$('#termTerm').combobox('select',attachment.term.term);
						
						
						$('#id').val(attachment.term.productId);
						$('#termId').val(attachment.id);
					} else {
						/** 操作失败* */
						$.messager.alert('提示信息', resMsg, 'error');
					}
			...
			function() {
				...
				var end=$("#endDate").datebox('getValue');
				  ...
				  var beginDate = new Date(startDate.replace(/\-/g, "\/"));
			        ...
			        if(!$.isEmpty(startDate) && !$.isEmpty(end) && beginDate >= endDate){
			            $.messager.alert('警告','起售日期不能大于停售日期!','warning');
			            return false;
			        }
