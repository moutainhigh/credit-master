...
/**
 * 证大P2P(直销) 计算器 版本 v1.0
 * 
 * @author Ivan
 *
 */
@Component("FS_00001_v1")
public class ZDCalculatorVOne extends ZDCalculatorBase implements ICalculator {

	@Override
	public BigDecimal getOnetimeRepaymentAmount(Long loanId, Date currDate, List<LoanRepaymentDetail> repayList) {
		System.out.println("证大P2P(直销) 计算器 版本 v1.0");
	...
	public void updateRate(LoanBase loanBase, LoanInitialInfo loanInitialInfo,
		...
		if (loanProduct.getTime() <= 12 && !loanInitialInfo.getLoanType().equals(LoanTypeEnum.薪生贷.getValue()) || 
			...
			loanProduct.setManageRateForPartyC(BigDecimal.ZERO);// 丙方管理费
				...
				((LoanTypeEnum.随薪贷.getValue().equals(loanInitialInfo.getLoanType()) || LoanTypeEnum.公积金贷.getValue().equals(loanInitialInfo.getLoanType()) || 
				LoanTypeEnum.网购达人贷B.getValue().equals(loanInitialInfo.getLoanType()) || LoanTypeEnum.网购达人贷A.getValue().equals(loanInitialInfo.getLoanType()) || LoanTypeEnum.保单贷.getValue().equals(loanInitialInfo.getLoanType()) || 
				LoanTypeEnum.淘宝商户贷.getValue().equals(loanInitialInfo.getLoanType())) && loanProduct.getTime()==18)) {
