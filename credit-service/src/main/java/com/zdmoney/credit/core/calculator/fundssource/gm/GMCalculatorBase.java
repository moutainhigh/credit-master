 ...
 */
@Component("FS_00004")
public class GMCalculatorBase extends CalculatorBase {
	...
	public void updateRate(LoanBase loanBase, LoanInitialInfo loanInitialInfo, LoanProduct loanProduct,ProdCreditProductTerm prodCreditProductTerm) {
		...
		loanProduct.setRisk(BigDecimal.ZERO);//风险金
	...
	public int getLoanType(String loanTypeStr) {
		 ...
		 int loanType = "薪生贷".indexOf(loanTypeStr) >= 0 ? 1 :
             ("学历贷".indexOf(loanTypeStr) >= 0 ? 2 :
             ("网购达人贷A、网购达人贷B".indexOf(loanTypeStr) >= 0 ? 3 :
             ("淘宝商户贷、保单贷、公积金贷".indexOf(loanTypeStr) >= 0 ? 4 :
             ("随房贷、随房贷A、随房贷B、随车贷、卡友贷、随薪贷".indexOf(loanTypeStr) >= 0 ? 5 :
             ("随意贷、随意贷A、随意贷B、随意贷C".indexOf(loanTypeStr) >= 0 ? 6 :-1)))));
