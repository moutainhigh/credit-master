package com.zdmoney.credit.core.calculator.fundssource.ljs;
 ...
 *
 */
@Component("FS_00021_v2")
public class LJSCalculatorVTwo extends LJSCalculatorBase implements ICalculator {
	private static final Logger logger = Logger.getLogger(LJSCalculatorVTwo.class);
	@Autowired
	...
	public BigDecimal getOnetimeRepaymentAmount(Long loanId, Date currDate, List<LoanRepaymentDetail> repayList) {
		System.out.println("陆金所银行 计算器 版本 v2.0");
	...
	public void updateRate(LoanBase loanBase, LoanInitialInfo loanInitialInfo,
		...
		try {
			//读取excel文件获取数据
			String excelPath = sysParamDefineService.getSysParamValue("system.thirdparty", "excelPathLJS");
			...
			logger.info("☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆陆金所银行updateRate错误",e);
		}
	}
	...
	public Map<String, Object> createLoanRepaymentDetail(LoanBase loanBase, LoanInitialInfo loanInitialInfo, LoanProduct loanProduct) {
		try {
			//读取excel文件获取数据
			String excelPath = sysParamDefineService.getSysParamValue("system.thirdparty", "excelPathLJS");
			logger.info("☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆陆金所计算器excel路径："+excelPath);
			...
			loanRepaymentDetailLufaxService.deleteByLoanId(loanBase.getId());
			for (int i = 0; i < excelInfo.getExcelDetailList().size(); i++) {
				...
				loanRepaymentDetailLufaxService.createLufaxRepaymentDetail(c);
			}
		} catch (Exception e) {
			logger.info("☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆陆金所银行生成还款计划错误",e);
			throw new PlatformException("☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆陆金所银行生成还款计划错误");
		}
		return null;//不用调整精度
	}
