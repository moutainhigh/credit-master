...
@Service
public class JimuheziPaymentRisk4PMJob {
	private static Logger logger = Logger.getLogger(JimuheziPaymentRisk4PMJob.class);
    ...
    private SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:30");
	...
	public void execute() {
    	String isJimuheziPaymentRisk4PM = sysParamDefineService.getSysParamValue("sysJob", "isJimuheziPaymentRisk4PM");
		if(!Const.isClosing.equals(isJimuheziPaymentRisk4PM)){
			Date today = Dates.getCurrDate();
			
			Date thisRepayDay = ToolUtils.getNextRepayDate(today);
			if (sysSpecialDayService.isRepayDayBeforeWorkDay(today)) {
				doRiskNoticeNotWorkday(thisRepayDay);
				doNoticeNormal(thisRepayDay);
			}
		}else{
			loanLogService.createLog("JimuheziPaymentRisk4PMJob", "info", "定时开关isJimuheziPaymentRisk4PM关闭，此次不执行", "SYSTEM");
			logger.warn("定时开关isJimuheziPaymentRisk4PM关闭，此次不执行");
		}

	}
	
	private void doRiskNoticeNotWorkday(Date date) {
		logger.info("积木提前结清的风险金预付发送邮件处理开始........");
		loanLogService.createLog("JimuheziPaymentRisk4PMJob", "info", "积木提前结清的风险金预付发送邮件处理开始........", "SYSTEM");
...
//    		returneterm = new BigDecimal((String)result.get("returneterm"));
        ...
        try {
			...
			loanLogService.createLog("JimuheziPaymentRisk4PMJob", "error", "积木提前结清的风险金预付发送邮件出错：" + (length > 1500 ? e.getMessage().substring(0, 1500) : e.getMessage()), "SYSTEM");
		}
        logger.info("积木提前结清的风险金预付发送邮件结束");
        loanLogService.createLog("JimuheziPaymentRisk4PMJob", "info", "积木提前结清的风险金预付发送邮件结束", "SYSTEM");
	}
	...
	private void doNoticeNormal(Date date) {
		try {
            ...
            if (datas.size() > 0) {
                ...
                loanLogService.createLog("JimuheziPaymentRisk4PMJob", "info", "积木提前结清通知发送邮件处理开始........", "SYSTEM");
                ...
                loanLogService.createLog("JimuheziPaymentRisk4PMJob", "info", "积木提前通知发送邮件结束", "SYSTEM");
            }
		} catch (Exception e) {
			logger.error("jimuhezi提前结清通知，数据附件处理出错:" + e.getMessage());
			int length = e.getMessage().length();
			loanLogService.createLog("JimuheziPaymentRisk4PMJob", "error", "jimuhezi提前结清通知，数据附件处理出错:" + (length > 1500 ? e.getMessage().substring(0, 1500) : e.getMessage()), "SYSTEM");
		}
	}
}
